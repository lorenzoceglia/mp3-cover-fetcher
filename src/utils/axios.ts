...
import type { DiscogsResponse } from "../typings/discogs";
import type { SpotifyTokenResponse } from "../typings/generics";
import type { ITunesSearchResponse } from "../typings/itunes";
import type { MusicBrainzResponse } from "../typings/musicBrainz";
import type { SpotifyResponse } from "../typings/spotify";
import { delay } from "./misc";

export async function fetchFromiTunes(terms: string[]) {
	for (const term of terms) {
		try {
			...
			if ([403, 429].includes(err as number)) await delay(2000);
		}
	}
	return null;
}
